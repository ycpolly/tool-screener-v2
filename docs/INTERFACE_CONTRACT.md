...
interface ScreenerParams {
  ...
  minVolExpansionRatio?:     number          // 當日量能擴增倍數（預設 1.0，動能攻擊要求 1.5）

  // K棒形態與影線
  checkTightConsolidation?:  boolean         // 狹幅震盪打底
  tightChgMin?:              number          // 狹幅打底漲跌幅下限（%）（預設 -1.5%）
  tightChgMax?:              number          // 狹幅打底漲跌幅上限（%）（預設 +2.0%）
  ...
  checkAvoidLongUpperShadow?: boolean        // 排除長上影線避雷針 (上影線 <= 實體紅 K 0.3 倍)
...
> **重要說明**：
> 為確保量化選股之極致嚴謹度，防止使用者誤調破壞交易系統的底層防護，引擎內部設有 **8 大底層寫死 / 隱形靈魂條件**。
...
| **2** | **狹幅震盪打底**<br>*(固定區間)* | `-1.5% <= changePct <= +2.0%`<br>*(上限放寬至 +2.0%)* | **底部蓄勢** | 🔹 **UI 標籤**：`狹幅震盪打底 (漲跌幅 -1.5% ~ +2.0%)`<br>💡 *底層固定區間：涵蓋蓄勢末端初步轉折表態* |
...
| **6** | **排除長上影線**<br>*(嚴化避雷針比例)* | `upperShadow > realBody * 0.3`<br>*(上影線 > 實體 0.3 倍則剔除)* | **動能攻擊<br>洗盤起漲<br>跌深反轉** | 🔹 **UI 標籤**：`排除長上影線 (上影線 ≤ 實體紅 K 0.3 倍)`<br>💡 *底層嚴格比例：根除假突破與隔日沖倒貨賣壓* |
| **7** | **帶量攻擊擴增倍數**<br>*(量能倍數)* | `volume >= vMa5 * 1.5`<br>*(需達 5 日量均 1.5 倍)* | **動能攻擊** | 🔹 **UI 標籤**：`當日帶量攻擊 (當日量 ≥ 5日量均 × 1.5)`<br>💡 *底層強制倍數：過濾微幅帶量之假突破* |
| **8** | **法人買超確認**<br>*(籌碼護航)* | 需具備外資 / 主力 / 投信買超標籤 | **底部蓄勢** | 🔹 **UI 標籤**：`法人買超確認 (獲外資/主力/投信買超支撐)`<br>💡 *底層強制條件：杜絕法人倒貨之破底死魚股* |
...
- **狹幅震盪打底**：UI 顯示 `狹幅震盪打底 (漲跌幅 -1.5% ~ +2.0%)`。
- **排除長黑倒貨**：UI 顯示 `排除長黑倒貨 (實體跌幅 ≥ 1.5% 且收最低)`。
- **法人買超確認**：UI 顯示 `法人買超確認 (外資 / 主力 / 投信買超支撐)`。

##### ② 動能攻擊 (`MOMENTUM_BREAKOUT`)
- **實體攻擊紅 K**：UI 顯示 `實體攻擊紅 K (收盤 > 開盤 且 漲幅 ≥ 1.5%)`。
- **昨日量縮洗盤**：UI 顯示 `昨日成交量 < 昨日 5 日均量 (MV5)`。
- **帶量突破倍數**：UI 顯示 `當日帶量攻擊 (當日量 ≥ 5日均量 × 1.5)`。
- **排除長上影線**：UI 顯示 `排除長上影線避雷針 (上影線 ≤ 實體紅 K 0.3 倍)`。
