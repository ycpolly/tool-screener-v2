...
> **最後更新：2026-09-19**（新增「近日表現」Bottom Sheet：上半部通欄 Sparkline 技術走勢圖與下半部橫向時間軸 Carousel，支援頂部 AB 測試一鍵切換時間軸與三欄表格，完成 v0919.01 版號維護）
...
- [x] 個股近日表現 Bottom Sheet（Stock Lifecycle Bottom Sheet：點擊 Sparkline 走勢圖喚起；支援【時間軸】與【表格】雙模式 AB 測試即時切換；時間軸模式採上半部通欄 Sparkline + 下半部橫向時間軸 Carousel，由左至右過去 T-7 至最新 T-0 與走勢圖同向，卡片垂直整齊呈現日期、收盤價、幾何箭頭漲跌幅與策略模式標籤；表格模式採俐落三欄式排版；採用 `<Teleport to="body">` 與 `v-if="isOpen"` 延遲計算，達成毫秒級瞬開體驗）— 完成 2026-09-19（v0919.01）
- [ ] AvoidModal（避雷區，法人賣超）
- [ ] 個股快捷連結（籌碼/多空/資券/盤後）
