...
### 雲端兩波流更新排程（GitHub Actions）

自動化機器人於台股交易日（週一至週五）分兩波抓取官方公開資訊並部署：

| 排程波次 | 執行時間 (台灣時間 CST) | 抓取內容與更新範圍 |
| :--- | :--- | :--- |
| **第一波** | **16:38** | 抓取盤後三大法人、15 組官方熱門量能排行、0050/0051 成分股、處置股名單及 Yahoo 歷史 K 線，產出 5 大選股模式初步名單。 |
| **第二波** | **18:42** | 券商分點進出明細出爐，完整更新個股 1D / 3D / 5D 籌碼集中度與主力短沖避雷標籤。 |

- **手動觸發**：若遇特殊情況需立即重新爬取，可至 GitHub Actions 選擇 `Update Stock Pool` 點擊 `Run workflow` 啟動。
