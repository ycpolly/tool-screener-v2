...
| :--- | :--- | :--- | :--- | :--- |
| **第一波**<br>`first-wave` | **16:38**<br>*(GCP Scheduler 定時喚醒)* | 抓取 0050/0051 成分股、15 組官方熱門量能排行、TWSE 官方處置股、三大法人買賣超及 Yahoo 近 3 個月歷史 K 線，產出 **5 大選股模式初步名單與技術指標**（不含券商分點籌碼）。 | 約 50 秒 | **搶先掌握收盤強勢股**：台股 13:30 收盤後，三大法人與量能排行於 16:30 陸續出爐；但全台灣各「券商分點買賣超明細」官方尚未結算。此波搶先在 16:40 前產出最新技術面選股池，供盤後快速複盤。 |
| **第二波**<br>`chips-wave` | **18:42**<br>*(GCP Scheduler 定時喚醒)* | 在第一波基礎上，額外對全市場 468 檔個股抓取券商分點進出明細，注入 **1D / 3D / 5D 籌碼集中度 %** 與 **短沖/隔日沖主力分點名冊（凱基台北、富邦建國等）與買超佔比 %**。 | 約 90 秒 | **補齊籌碼透視與短沖避雷拼圖**：證交所與集保中心各券商分點進出大表通常於 18:00~18:30 後才完整出爐。此波於 18:42 執行，正式補齊大戶主力與短沖避雷數據，完成當日最終完整版資料庫。 |

- **主動喚醒機制（GCP Cloud Scheduler）**：
  - 由於 GitHub Actions 免費 cron 排程在傍晚尖峰易遭遇 3~5 小時排隊延遲，系統採用 GCP Cloud Scheduler（永久免費）於 16:38 與 18:42 發送 HTTP POST 請求至 GitHub 的 `workflow_dispatch` API。
