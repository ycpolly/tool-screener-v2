...
- [x] LINE 選股快照純文字排版精簡重構（Screener Snapshot Format 1：適配 LINE 窄對話泡泡，股票項目由單行改為雙行排版【首行代號名稱現價，次行以 ▲ / ▼ / 0 醒目幾何符號呈現漲跌點數與百分比】；全域時間精簡為 `MM/DD W HH:mm:ss 複製/資料`；模式標題升級為 `＝＝＝ 模式 (N) ＝＝＝`；區段改為 `【一般篩選】` 與 `【一鍵精選】`；項目間保持整齊空行，杜絕文字折行，提升閱讀流暢度）— 完成 2026-09-05
