...
- [x] 手機端模式主訴與調整參數垂直分行（ScreenerPanel Mobile Layout：手機端由橫排並列改為垂直分行【上層：模式主訴滿版通欄，下層靠右：調整參數 / 重設自訂按鈕組】，電腦端維持單行橫排；徹底釋放手機橫向寬度，根除長文案如「趨勢多頭、指標降溫後再度帶量攻擊起漲」在手機端尾字擠壓折行的問題）— 完成 2026-09-05
