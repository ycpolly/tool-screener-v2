...
- [ ] StockCard 標籤收斂去重視覺確認（由 Gemini 前端負責）：Claude 已於 `src/constants/category-urls.js` 的 `getStockCategoryItems` 實作母子標籤智慧收斂（個股若已具備外資1D/3D、主力1D/3D、投信3D/5D 等明確天數子標籤，自動過濾隱藏抽象母標籤外資買、主力買、投信買，徹底根除語義冗餘與重複超連結）。請前端確認 `StockCard.vue` 在手機端與電腦端的視覺排版與折行表現，確保樣式極簡流暢。
- [ ] RiskModal（空間與風控全貌）
- [ ] AvoidModal（避雷區，法人賣超）
- [ ] 個股快捷連結（籌碼/多空/資券/盤後）
