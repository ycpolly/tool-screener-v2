    ...
    <div v-else class="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
      ...
      <div class="space-y-1">
        <div class="flex items-center gap-2">
          <span class="font-bold text-xs uppercase px-2 py-0.5 rounded border" :class="regimeBadgeClass">
            {{ regime?.badge || '風控判定' }}
          </span>
        ...
        <p v-if="regime?.subtitle" class="text-xs text-base-content/65 leading-normal">
          {{ regime.subtitle }}
        </p>
      </div>

      <!-- 右側：加權指數 (TAIEX) 與 櫃買指數 (OTC) 迷你數據卡 -->
      <div class="flex items-center gap-4 pt-2 md:pt-0 border-t md:border-t-0 border-base-300/40 font-numeric text-xs">
        <!-- 加權指數 -->
        <div v-if="taiex" class="flex flex-col items-start md:items-end">
          <div class="text-base-content/60 font-sans text-[11px]">{{ taiex.name || '加權指數' }}</div>
          <div class="flex items-baseline gap-1.5">
            <span class="font-bold text-sm text-base-content">{{ formatIndexPrice(taiex.price) }}</span>
            <span class="font-semibold text-xs" :class="taiex.changePct >= 0 ? 'text-rise' : 'text-fall'">
        ...
        <div v-if="otc" class="flex flex-col items-start md:items-end pl-3 border-l border-base-300/40">
          <div class="text-base-content/60 font-sans text-[11px]">{{ otc.name || '櫃買指數' }}</div>
          <div class="flex items-baseline gap-1.5">
            <span class="font-bold text-sm text-base-content">{{ formatIndexPrice(otc.price) }}</span>
            <span class="font-semibold text-xs" :class="otc.changePct >= 0 ? 'text-rise' : 'text-fall'">
